...
import { Button, Field, Input, Modal, Toast } from '../components/ui/Primitives'
...
export default function AuthPage({ mode = 'login' }) {
  const { login, signup, loginWithGoogle, resetPassword, beginSignInTwoFactor, finishSignInTwoFactor, demoMode } = useAuth()
  ...
  const onSubmit = async (data) => {
    try {
      if (mode === 'login') { const result = await login(data.email, data.password); if (result?.requiresTwoFactor) { setTwoFactor(result); return } navigate(target, { replace: true }) }
  ...
  const sendTwoFactorCode = async () => { try { const result = await beginSignInTwoFactor(twoFactor.resolver); setVerificationId(result.verificationId) } catch (error) { setToast({ message: error.message, type: 'error' }); setTwoFactor(null) } }
  const completeTwoFactor = async (event) => { event.preventDefault(); try { const code = new FormData(event.currentTarget).get('code'); await finishSignInTwoFactor(twoFactor.resolver, verificationId, code); navigate(target, { replace: true }) } catch (error) { setToast({ message: error.message, type: 'error' }) } }
  return <div className="auth-page"><AuthArt /><main className="auth-main"><div className="auth-mobile-brand"><span className="brand-mark"><span /></span> moneyflow</div><div className="auth-form-wrap"><Link className="auth-back" to={mode === 'signup' ? '/login' : mode === 'reset' ? '/login' : '#'} onClick={(event) => mode === 'login' && event.preventDefault()}>{mode !== 'login' && <><ArrowLeft size={16} /> Back to sign in</>}</Link><span className="auth-eyebrow">{view.eyebrow}</span><h1>{view.heading}</h1>{mode === 'reset' && <p className="auth-description">Enter the email tied to your account and we’ll send you a secure reset link.</p>}
    ...
    </form><p className="auth-switch">{view.foot} <Link to={mode === 'login' ? '/signup' : '/login'}>{view.link}</Link></p>{demoMode && mode === 'login' && <p className="demo-login"><ShieldCheck size={15} /> Demo mode — use any valid email and 6+ character password.</p>}</div><Modal open={Boolean(twoFactor)} onClose={() => { setTwoFactor(null); setVerificationId(null) }} title="Two-step verification">{verificationId ? <form className="stack-form" onSubmit={completeTwoFactor}><p className="soft-note">Enter the code we sent to your registered phone.</p><Field label="Verification code"><Input name="code" inputMode="numeric" autoComplete="one-time-code" required /></Field><div className="modal-actions"><Button variant="secondary" onClick={() => setVerificationId(null)}>Back</Button><Button type="submit">Verify</Button></div></form> : <div className="stack-form"><p className="soft-note">Your account is protected with a second step.</p><div id="login-recaptcha-container" /><div className="modal-actions"><Button variant="secondary" onClick={() => setTwoFactor(null)}>Cancel</Button><Button onClick={sendTwoFactorCode}>Send code</Button></div></div>}</Modal><Toast {...(toast || {})} onClose={() => setToast(null)} /></main></div>
}
